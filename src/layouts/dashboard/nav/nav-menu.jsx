...
const AccordionSummary = styled(({isHasArrow, ...props}) => (
  <MuiAccordionSummary
    expandIcon={isHasArrow ? <ArrowDownIcon width={12} height={10}/>: null}
    {...props}
  />
))(({ theme }) => ({
  display: 'flex',
...
export default function NavMenu({expanded}) {
  ...
  return (
    <>
    {
      navConfig.map( (item, index) => (<Accordion key={index} expanded={expandedDetail === `panel1${index}`} onChange={handleChange(`panel1${index}`)}>
       <EffectBtn
